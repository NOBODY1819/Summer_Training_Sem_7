...
public:     //Solution 1
    ...
    // int rangeSumBST(TreeNode* root, int low, int high) {
    //     vector<int> tree;
    //     inorder(root,tree,low,high);
    //     return accumulate(tree.begin(),tree.end(),0);
    // }
    // void inorder(TreeNode* temp, vector<int>& tr,int l, int h){
    //     if(temp!=NULL){
    //         inorder(temp->left, tr, l, h);
    //         if(temp->val >=l && temp->val <=h) tr.push_back(temp->val);
    //         inorder(temp->right, tr,l,h);
    //     }
    // }
};
